...
| Task | Audit | Files | Acceptance | Status |
| --- | --- | --- | --- | --- |
| **R4** Populate `Niche.parent_niche_id` / `depth` on promotion | #3 | `niche_canonicalization.py` | After a depth-3 drill, `_niche_path()` returns ≥3 nodes; handoff package shows full path; parent lookup filters `status == PROMOTED` | |
