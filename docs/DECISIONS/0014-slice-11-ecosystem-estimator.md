...
- `tests/integration/test_ecosystem_estimator.py` (7, real Postgres): all
  counting paths, dedup, follower filtering, skip logic, empty cases.
- Non-DB suite: 304 passed. DB suite: 7 passed. ruff: clean.
...
- The `target_band_creator_count` relies on `follower_count` from the
  adapter metadata. For yt-dlp video search results, this is typically null.
  A future enhancement could resolve each unique channel_id to get
  subscriber counts via a profile fetch (one yt-dlp call per channel).
