...
@pytest.mark.asyncio
async def test_load_observations_orders_by_confidence(clean_db: AsyncSession):
    """_load_observations caps at 10 rows; with no ORDER BY which 10 of N came
    back was undefined — the "representative" observations shown in a
    decision-ready dossier could change between identical calls purely from
    physical row order. Must return the top 10 by confidence, highest first."""
    session = clean_db
    ...
    # 15 observations with distinct, shuffled confidence scores — the
    ...
    for i, score in enumerate(scores):
        ...
        session.add(ProblemClusterMember(
            cluster_id=cluster.id, observation_id=obs.id, similarity_score=0.5,
        ))
    await session.flush()
