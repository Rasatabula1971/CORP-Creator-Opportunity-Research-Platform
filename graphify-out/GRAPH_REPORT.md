...
## Communities (288 total, 66 thin omitted)
...
Cohesion: 0.14
Nodes (32): CreatorStatus, str, DecisionType, creator_status_for_dossiers(), mirror_creator_status(), AsyncSession, Gate A logic — human review decisions with state machine transitions — and the…, Record a Gate A decision and transition the creator's status. Raises… (+24 more)

### Community 2 - "test_scoring_engine.py"
Cohesion: 0.03
Nodes (121): CompetitorStrength, compute_hash(), compute_score(), get_score_band(), growth_ratio(), load_scoring_rules(), Any, Evidence depth input: observation counts weighted by how they were obtained. (+113 more)
...
Cohesion: 0.08
Nodes (49): ProviderError, ProviderUnavailableError, Exception, Transient failure (5xx, timeout, transport) that outlasted the retry budget., FairProvider, _model_label(), Any, LLMProvider backed by an in-process FAIR router. ``router`` is a… (+41 more)

### Community 5 - "ConfidenceBand"
Cohesion: 0.17
Nodes (24): ConfidenceBand, str, compute_confidence_band(), Any, Confidence banding driven by YAML thresholds when provided. Args: thresholds:…, _make_client(), AsyncClient, asyncio (+16 more)

### Community 6 - "ClusteringConfig"
Cohesion: 0.13
Nodes (32): _build_clusters(), cluster_observations(), ClusteringConfig, _generate_label(), _pick_representative(), datetime, ndarray, Problem observation clustering via UMAP + HDBSCAN with c-TF-IDF labeling. (+24 more)
...
### Community 8 - "Niche"
Cohesion: 0.09
Nodes (66): CampaignStatus, str, Niche, Canonical niche identity. A broad domain (e.g. "Fitness") is a discovery input,…, _create_campaign_and_niche(), _create_creator(), _create_evidence(), asyncio (+58 more)
...
Cohesion: 0.04
Nodes (40): describe_http_error(), _extract_channel_id(), _http_error_reason(), _parse_iso8601_duration(), Any, BaseException, datetime, HttpError (+32 more)

### Community 11 - "IntentPipeline"
Cohesion: 0.22
Nodes (14): IntentPipeline, Classifies commercial intent for each active ProblemCluster. Runs inside the…, FakeProvider, asyncio, AsyncSession, Integration tests for IntentPipeline against real Postgres., Create creator → evidence → observations → cluster with members., _seed_cluster() (+6 more)
...
Cohesion: 0.12
Nodes (22): extract_observations(), extract_observations_batch(), ExtractedObservation, _parse_observation(), Any, Per-comment and batch problem/question/pain extraction via LLM., DTO for a single extracted problem observation., Extract problem observations from a single comment. Raises ``LLMCallError``… (+14 more)

### Community 15 - "ComplianceStatus"
Cohesion: 0.04
Nodes (112): create_evidence(), AsyncSession, AccessMethod, ComplianceStatus, EvidenceOrigin, EvidenceType, infer_evidence_type(), str (+104 more)
...
Cohesion: 0.07
Nodes (36): Groq free tier via its OpenAI-compatible chat completions endpoint. JSON mode…, classify_quota_error(), _finish_reason(), GeminiProvider, Any, BaseException, LLM provider abstraction with deterministic call logging., Why the first candidate stopped, for error messages; None when unknown. (+28 more)
...
Cohesion: 0.16
Nodes (50): NicheCandidate, NicheCandidateStatus, str, The Niche the parent candidate resolved to (PROMOTED or MERGED -- lineage…, Broad topic -> capability fan-out -> LLM synthesis -> recurse. ``provider`` is…, See :func:`registry_fresh` -- the drill engine and the Level 0 trend scan must…, RecursiveNicheDiscovery, _config() (+42 more)
...
Cohesion: 0.12
Nodes (30): AudienceInteraction, ContentType, InteractionType, str, ContentItemCreate, ContentItemResponse, InteractionCreate, InteractionResponse (+22 more)
...
### Community 24 - "Creator"
Cohesion: 0.11
Nodes (42): get_clusters(), Base, generate_uuid(), TimestampMixin, CampaignNiche, How one niche performed within one campaign (§12). Never overwrites Niche's own…, Competitor, An existing product/creator/workaround already serving a problem cluster's… (+34 more)

### Community 25 - ".select"
Cohesion: 0.14
Nodes (26): archive_creator(), generate_persisted_dossier(), _generate_product_ideas_best_effort(), get_campaign(), get_competitors(), get_creator(), get_dossier(), get_evidence() (+18 more)

### Community 27 - "MarketplaceAdapter"
Cohesion: 0.05
Nodes (54): Return ``prefix`` + 16 hex chars derived from ``parts`` (case-insensitive)., stable_id(), _extract_price(), _html_to_text(), _is_retryable(), MarketplaceAdapter, _parse_etsy_api_response(), _parse_etsy_listings() (+46 more)
...
Cohesion: 0.07
Nodes (58): BaseSettings, provider_health(), Diagnose the active LLM provider. Returns the active provider's name and, when…, Settings, build_provider(), _configured_members(), _gemini(), _groq() (+50 more)
...
Cohesion: 0.08
Nodes (41): ResearchRun, RunScope, Collect all data for a creator and persist to DB. Args: identifier: Platform-…, One query against one source, under ``campaign_id``. Returns the run. Never…, Fan out ``query`` across all healthy niche adapters under ``campaign_id``.…, Campaign research batch (Slice 15). The last mile of the niche pipeline: drive…, Make ``run`` safe to touch after a researcher crash.…, Clustering pipeline — embed observations, cluster, persist to DB, unify topics. (+33 more)
...
Cohesion: 0.14
Nodes (29): CreatorOnboarder, OnboardConfig, Any, AsyncSession, Search a niche and return every unique channel (no band filter, no cap).…, One deduplicated, batched subscriber-count lookup for the whole run., ChannelEnricher, Protocol (+21 more)
...
Cohesion: 0.10
Nodes (31): as_bool(), as_float(), as_int(), Any, Defensive coercion of untrusted LLM JSON values. Model output is not shape-…, Coerce ``value`` to float, returning ``default`` if it isn't numeric., Coerce ``value`` to int, returning ``default`` if it isn't a whole number.…, Coerce ``value`` to bool, treating the string "false"/"0"/"no"/… as False.… (+23 more)

### Community 80 - "AppStoreAdapter"
Cohesion: 0.07
Nodes (33): AppStoreAdapter, _parse_iso(), _parse_review_feed(), Any, AsyncClient, datetime, retry, Parse an Apple RSS ``updated`` timestamp (ISO 8601), normalized to UTC. (+25 more)

### Community 81 - "StackExchangeAdapter"
Cohesion: 0.12
Nodes (28): AsyncClient, Collects questions (and their answers) from a Stack Exchange site. Each…, StackExchangeAdapter, _answer(), _make_response(), mock_client(), _mock_resp(), fixture (+20 more)

### Community 82 - "NormalizedContent"
Cohesion: 0.06
Nodes (17): NormalizedContent, BaseModel, The one schema every adapter family emits. This is the Build item.…, Any, datetime, retry, ProblemProvider (CORP1 Stage 4/5, T2): "How do I X?" questions are a direct…, _ts() (+9 more)

### Community 83 - "ProviderExhaustedError"
Cohesion: 0.13
Nodes (30): ProviderExhaustedError, Provider health signals the pool acts on. Only these two trigger failover.…, A quota that will not clear inside one call's retry window. ``retry_after`` is…, Failover across free-tier providers. Same ``LLMProvider`` interface, so…, Clock, daily(), Fake, _pool() (+22 more)

### Community 84 - "test_watch_rescanner.py"
Cohesion: 0.12
Nodes (41): decide_resurface(), RuntimeError, Design §3.3 option C. Pure so it can be unit-tested exhaustively., A stage of the re-research chain failed; the message names it., RescanError, _d(), FakeDriller, FakeIdeator (+33 more)

### Community 85 - "CrowdfundingAdapter"
Cohesion: 0.07
Nodes (33): CrowdfundingAdapter, _parse_indiegogo_date(), _parse_indiegogo_response(), _parse_kickstarter_response(), Any, AsyncClient, datetime, retry (+25 more)
...
Cohesion: 0.16
Nodes (6): AsyncNetworkStream, Any, HTMLParser, Network backend that validates DNS at connection time. The pre-flight…, _SSRFSafeBackend, _TextAndLinks

### Community 89 - "test_registry_rescan.py"
Cohesion: 0.16
Nodes (46): find_due_watched_dossiers(), Every currently-active (never-superseded) WATCHING dossier whose niche's…, One re-scan pass. Regenerates the dossier for every due WATCHING dossier. A…, A rescanner plus the cleanup for whatever it was built around (the LLM…, rescan_watched_dossiers(), RescannerHandle, _due_watching(), FakeRescanner (+38 more)

### Community 90 - "DossierStatus"
Cohesion: 0.21
Nodes (28): DossierStatus, str, Denormalized mirror of the latest HumanDecision for fast dashboard queries.…, _make_client(), AsyncClient, asyncio, AsyncSession, LogCaptureFixture (+20 more)

### Community 91 - "AdapterFamily"
Cohesion: 0.05
Nodes (37): AdapterFamily, str, What an adapter's output attaches to., is_public_url(), _normalize(), parse_page(), AsyncClient, Extract title, description, visible text, and classified outbound links. (+29 more)

### Community 92 - "routes_ops.py"
Cohesion: 0.08
Nodes (55): BackgroundTasks, add_account(), CampaignPipelineRequest, create_campaign(), create_creator(), CreatorCreateRequest, discovery_status(), DiscoveryRunRequest (+47 more)

### Community 93 - "build_adapter"
Cohesion: 0.05
Nodes (49): AdapterConfigError, build_adapter(), build_search_adapter(), Exception, Adapter selection — builds a SourceAdapter for a platform name from settings., The requested platform is unknown or not configured., Adapter for creator DISCOVERY via keyword search (``ytsearchN:`` syntax).…, _default_factory() (+41 more)

### Community 94 - "DossierGenerator"
Cohesion: 0.05
Nodes (87): ColumnElement, DossierEvidence, One evidence row's membership in one dossier's CORP2 handoff trail. Mirrors…, ProductIdea, ProductIdeaComplexity, ProductIdeaEvidence, ProductIdeaType, str (+79 more)
...
Cohesion: 0.17
Nodes (33): HumanDecision, Append-only. Every decision is a new row — never update or delete., build_handoff_package(), EvidenceTrailEntry, HandoffPackage, NichePathEntry, AsyncSession, CORP1 -> CORP2 Handoff Package (CORP1 Stage 5, T9). Per the Stage 3 spec's own… (+25 more)
...
Cohesion: 0.19
Nodes (22): OpportunityScore, Any, AsyncSession, Scores every active ProblemCluster for a creator, then aggregates per creator., Load the strength of every Competitor row known for this cluster. Independent…, ScoringPipeline, asyncio, AsyncSession (+14 more)

### Community 101 - "test_runs.py"
Cohesion: 0.13
Nodes (20): str, Stored as plain strings on ResearchRun.status., RunStatus, Advance the creator to ``working`` for the block, then ``done`` on success. The…, Decide a run's terminal status from its stats., resolve_status(), stage(), FakeSession (+12 more)

### Community 102 - "record_query"
Cohesion: 0.14
Nodes (26): str, One executed search, recorded under its ResearchRun (§12, ADR-006). This is the…, ResearchQuery, ResearchQueryStatus, find_queries(), AsyncSession, datetime, Research query ledger — the persistence service for ResearchQuery (§13). Two… (+18 more)
...
Cohesion: 0.20
Nodes (23): NicheCandidateEvidence, One evidence row's membership in one candidate, with its similarity to the…, NicheCandidateGenerator, _candidates(), FakeEmbedder, NamingProvider, asyncio, AsyncSession (+15 more)

### Community 105 - "Evidence"
Cohesion: 0.09
Nodes (42): Evidence, Append-only evidence store. No UPDATE or DELETE — ever., NicheDiscoveryCollector, AsyncSession, Niche discovery, light (§15 Stage A): one source → stored evidence. Given a…, _evidence_supporting(), _fetch_method(), _gather_safely() (+34 more)

### Community 106 - "test_campaign_research.py"
Cohesion: 0.12
Nodes (19): Real end-to-end liveness probe: enumerate providers, then run a trivial schema-…, T, _batch(), _ExpiredAttributeError, _FakeCreator, _FakeSession, patch, RuntimeError (+11 more)
...
Cohesion: 0.08
Nodes (22): PingResult, Outcome of a FAIR end-to-end liveness probe. ``ok`` is only true when at least…, client(), _FakeFair, fixture, Tests for /providers/health — the endpoint that reports the active LLM provider…, A bare Gemini/Groq or a PooledProvider isn't a FairProvider — no ping is run,…, PooledProvider isn't FAIR, but it does have member_names — verify that member… (+14 more)

### Community 111 - "test_micro_niches.py"
Cohesion: 0.24
Nodes (28): normalize_label(), _cluster(), _creator(), parametrize, _queue(), Creator-first discovery, step 1: audience clusters → approval queue., Re-research supersedes a creator's clusters and makes new ones. The creator…, Approval-first: generating the queue must not touch the drill engine. (+20 more)
...
### Community 113 - "Dossier"
Cohesion: 0.19
Nodes (28): Work function for the Research More decision outcome (T8, completed by R12a).…, _run_research_more(), Dossier, One decision-ready dossier for one creator x niche x opportunity., Gate, _create_candidate(), _create_dossier_prereqs(), asyncio (+20 more)

### Community 114 - "RedditAdapter"
Cohesion: 0.07
Nodes (35): unarchive_creator(), Any, AsyncClient, datetime, retry, Newest posts for ``r/<sub>``, ``u/<user>`` or a bare subreddit name., Full comment tree for one post. Collapsed 'more' stubs are skipped., Collects posts and full comment trees from a subreddit or user profile. (+27 more)
...
Cohesion: 0.23
Nodes (14): _is_enrichable_channel_id(), Only real YouTube channel IDs (``UC`` + 22 chars) can be looked up via the Data…, _Niche, _onboarder(), No-DB unit tests for the onboarding subscriber-count enrichment path. These…, test_enrich_all_dedups_across_niches(), test_enrich_all_failure_is_graceful(), test_enrich_all_no_enrichable_ids_skips_call() (+6 more)
...
### Community 121 - "multi_discovery.py"
Cohesion: 0.08
Nodes (26): Write the raw normalized items as JSONL; return a data-path-relative reference.…, MultiSourceDiscovery, Any, Multi-source niche discovery — fans a query across all niche adapters. Given a…, Orchestrates niche discovery across multiple adapters. Usage:: disco =…, _to_evidence(), Shared filename-safe slug helper for acquisition warmstore paths., slugify() (+18 more)

### Community 122 - "NicheLifecycleStatus"
Cohesion: 0.26
Nodes (18): NicheLifecycleStatus, The cycle from §7: CANDIDATE precedes verification; ACTIVE/EXPAND then cycle…, NicheVerifier, AsyncSession, VerifyConfig, VerifyResult, asyncio, AsyncSession (+10 more)

### Community 123 - "ProblemCluster"
Cohesion: 0.11
Nodes (22): ProblemCluster, CompetitivePipeline, Discovers competitors for each ProblemCluster via LLM analysis., Run competitive discovery for all clusters belonging to a creator., ClusterContext, CreatorContext, Frequency of the most recent superseded cluster with the same label., Everything about a creator the per-cluster scoring needs, loaded once. (+14 more)
...
Cohesion: 0.14
Nodes (40): NicheAlias, Alternate wording that resolves to one canonical Niche. Existence of this table…, CanonConfig, NicheCanonicalizer, AsyncSession, FakeEmbedder, asyncio, AsyncSession (+32 more)
...
Cohesion: 0.11
Nodes (17): DiscoveryHandle, DiscoveryScanScheduler, async_sessionmaker, AsyncSession, A provider plus how to release it once the tick is done., In-process loop running one :func:`run_discovery_pass` per tick. Start it from…, ProviderFactory, _noop() (+9 more)
...
Cohesion: 0.10
Nodes (16): BusyCheck, _campaign_for_niche(), async_sessionmaker, AsyncSession, datetime, Exception, Protocol, Registry Re-scan Scheduler (CORP1 Stage 5, T10). Architecture invariant (Stage… (+8 more)
...
### Community 136 - "trend_scan.py"
Cohesion: 0.17
Nodes (7): RuntimeError, CORP1 Step 1, Level 0 — the autonomous trend scan. The drill engine…, Return up to ``limit`` topics ready to drill, best first., Best-effort momentum score per topic. Missing keys mean no signal, which the…, Epoch seconds of each topic's last research pass, when known., No live momentum signal, and the catalogue forbids rotation fallback., TrendSignalUnavailableError
...
Cohesion: 0.15
Nodes (18): What kind of research this run is (§11), distinct from RunScope, which…, RunType, InvalidResearchRunError, Exception, Validation for ResearchRun's run_type/reference invariants (§11). Enforces the…, validate_run_type(), _cosine(), Any (+10 more)
...
### Community 144 - "WatchRescanner"
Cohesion: 0.12
Nodes (15): build_watch_rescanner(), content_fingerprint(), CreatorResearcher, Ideator, NicheDriller, Any, AsyncSession, Protocol (+7 more)
...
### Community 146 - "integration/test_cluster_pipeline.py"
Cohesion: 0.27
...
Cohesion: 0.10
Nodes (30): Why a pass returned what it did — surfaced in the run record so a scan that…, A broad topic cleared to enter recursive discovery., SeedTopic, TrendScanStats, get_or_create_autonomous_campaign(), The standing campaign for unattended passes, created on first use. Matched…, One full pass. Caller owns the transaction. ``topics`` overrides the trend scan…, run_discovery_pass() (+22 more)

### Community 149 - "ClusterPipeline"
Cohesion: 0.11
Nodes (17): ClusterPipeline, _cosine_to_centroid(), ndarray, Merge cluster labels into the creator's ContentItem.topics list. Produces a…, Cosine similarity of each row to the centroid, in [0, 1] (clipped)., Embeds ProblemObservations, clusters them, and persists results. A new run for…, ClusterResult, Output of the clustering step. (+9 more)
...
### Community 151 - "discovery_scan.py"
Cohesion: 0.18
Nodes (14): build_momentum_provider(), DiscoveryPassStats, momentum_readiness(), Any, The autonomous discovery crawler — CORP1 Step 1 with no user input. One pass…, The TrendProvider the scanner ranks with, or None to rank by rotation. One…, Outcome of one pass, for the log line and the API response., (source, available, why-not) for the configured momentum source. No network:… (+6 more)
...
Cohesion: 0.20
Nodes (15): FakeProvider, asyncio, AsyncSession, Integration tests for IntelligencePipeline against real Postgres., Comment → Evidence → ProblemObservation FK chain is intact., Even if no comments exist, topic classification should run., Deterministic provider returning canned responses., Create creator + content item + interaction + evidence for testing. (+7 more)
...
Cohesion: 0.08
Nodes (35): _close_quietly(), create_app(), _discovery_busy(), DiscoveryProviders, _job_busy(), lifespan(), _mark_orphaned_runs(), AsyncSession (+27 more)

### Community 218 - "_strip_code_fence"
Cohesion: 0.20
Nodes (8): Free models often wrap JSON in a markdown fence even when told not to., _strip_code_fence(), _float_header(), _is_transient(), Any, BaseException, Classify a 429. ``daily`` only when the request budget is actually spent;…, test_strip_code_fence_leaves_plain_text_alone()
...
### Community 234 - "DiscoveryDeferredError"
Cohesion: 0.33
Nodes (6): _noop_close(), DiscoveryDeferredError, Exception, Skip this tick without counting it as a failure (providers cooling)., test_scheduler_defers_without_counting_a_failure(), factory()
...
### Community 270 - "conftest.py"
Cohesion: 0.22
Nodes (9): clean_db(), db_session(), _fresh_app_engine(), AsyncSession, fixture, pytest_sessionfinish(), corp.database's pooled asyncpg engine binds its connections to the event loop…, Truncate all tables before a test, then yield a fresh session. (+1 more)

### Community 271 - "collector.py"
Cohesion: 0.08
Nodes (39): embedding_to_bytes(), _get_store(), mirror_content_items(), mirror_evidence(), mirror_interactions(), mirror_metrics(), mirror_observations(), mirror_research_queries() (+31 more)

### Community 275 - "CampaignResponse"
Cohesion: 0.67
Nodes (3): CampaignCreate, CampaignResponse, BaseModel

### Community 276 - "CreatorNicheResponse"
Cohesion: 0.67
Nodes (3): CreatorNicheCreate, CreatorNicheResponse, BaseModel

### Community 277 - "_isoformat_nested"
Cohesion: 0.67
Nodes (3): _isoformat_nested(), Any, JSON-safe rendering: datetimes as ISO 8601 strings, including ones nested…
...
Cohesion: 0.07
Nodes (42): _close(), _commit_or_rollback(), _embedder_factory(), _singleton(), Any, In-process background jobs for long-running pipelines. The console needs to…, Persist whatever a crashing pipeline flushed (e.g. a failed ResearchRun). If…, Queue micro-niche suggestions from freshly researched audience clusters. Best-… (+34 more)

### Community 283 - "test_dossier_pipeline.py"
Cohesion: 0.31
Nodes (14): CompetitorType, str, asyncio, AsyncSession, Integration tests for DossierGenerator against real Postgres., Seed a creator with clusters, signals, and scores — full pipeline output., _seed_full(), test_dossier_contains_competitors() (+6 more)
...
- **Why does `ComplianceStatus` connect `ComplianceStatus` to `test_captions.py`, `ConfidenceBand`, `test_endpoints.py`, `Niche`, `YouTubeAdapter`, `IntentPipeline`, `integration/test_cluster_pipeline.py`, `test_youtube_trends_adapter.py`, `test_niche_discovery.py`, `AcquisitionCollector`, `Creator`, `MarketplaceAdapter`, `test_dossier_pipeline.py`, `test_intelligence_pipeline.py`, `ResearchRun`, `AmazonReviewAdapter`, `PatreonSubstackAdapter`, `AppStoreAdapter`, `StackExchangeAdapter`, `NormalizedContent`, `test_watch_rescanner.py`, `CrowdfundingAdapter`, `GoogleTrendsAdapter`, `WikipediaAdapter`, `test_registry_rescan.py`, `AdapterFamily`, `build_adapter`, `DossierGenerator`, `SearchDemandAdapter`, `HackerNewsAdapter`, `test_corp2_export.py`, `OpportunityScore`, `test_niche_candidates.py`, `Evidence`, `Dossier`, `RedditAdapter`, `multi_discovery.py`, `NicheLifecycleStatus`, `ProblemCluster`, `test_niche_canonicalization.py`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `LLMProvider` connect `LLMProvider` to `DiscoveryScanScheduler`, `ResearchOrchestrator`, `test_fair_provider.py`, `test_competitive_pipeline.py`, `IntelligencePipeline`, `ScriptedProvider`, `IntentPipeline`, `test_intent_classifier.py`, `RunType`, `extract_observations`, `providers/registry.py`, `test_niche_discovery.py`, `classify_topics`, `test_discovery_scan.py`, `Embedder`, `Creator`, `jobs.py`, `discovery_scan.py`, `build_provider`, `PooledProvider`, `test_intelligence_pipeline.py`, `ResearchRun`, `LLMCallError`, `ProviderExhaustedError`, `app.py`, `DossierGenerator`, `test_niche_candidates.py`, `Evidence`, `test_niche_naming.py`, `test_groq_provider.py`, `ProblemCluster`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `ResearchRun` connect `ResearchRun` to `ResearchOrchestrator`, `test_endpoints.py`, `Niche`, `IntelligencePipeline`, `IntentPipeline`, `CampaignResearchBatch`, `RunType`, `NicheQualifier`, `collector.py`, `WatchRescanner`, `test_watching_dossiers.py`, `integration/test_cluster_pipeline.py`, `ComplianceStatus`, `test_niche_discovery.py`, `ClusterPipeline`, `AcquisitionCollector`, `Embedder`, `Creator`, `.select`, `test_dossier_pipeline.py`, `.record_step`, `routes.py`, `test_intelligence_pipeline.py`, `CreatorOnboarder`, `test_watch_rescanner.py`, `app.py`, `test_registry_rescan.py`, `routes_ops.py`, `DossierGenerator`, `OpportunityScore`, `test_runs.py`, `record_query`, `test_niche_candidates.py`, `Evidence`, `EcosystemEstimator`, `test_micro_niches.py`, `Dossier`, `multi_discovery.py`, `NicheLifecycleStatus`, `ProblemCluster`, `test_niche_canonicalization.py`, `Campaign`?**
